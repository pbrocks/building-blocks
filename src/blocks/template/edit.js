	...
	render() {
		...
		const dropZone = (
			<DropZone
				...
				label={ __( 'Drop to upload as artwork', 'building-blocks' ) }
			/>
		);
		...
		return (
			<Fragment>
				{ mediaID &&
 				...
 				{ isSelected && mediaID && (
					<InspectorControls>
						<PanelBody title={ __( 'template Settings', 'building-blocks' ) }>
							<TextareaControl
									...
									<Button onClick={ open }>
										{ ! mediaID ?
											...
											<Fragment>
												...
												<img className="wp-block-building-blocks-template__avatar-img" src={ mediaURL } alt={ mediaALT } />
											</Fragment>
										}
									</Button>
								) }
					...
					<div className="wp-block-building-blocks-template__content">
						<RichText
							...
							placeholder={
								...
								__( 'Write template name…', 'building-blocks' )
							}
						...
						<RichText
							...
							placeholder={
								// translators: placeholder text used for the quote
								__( 'Write template biography…', 'building-blocks' )
							}
							keepPlaceholderOnFocus
						/>
					</div>
